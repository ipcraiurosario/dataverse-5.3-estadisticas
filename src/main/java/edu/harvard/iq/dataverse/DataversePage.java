...
<<<<<<< HEAD
    ...
    public void updateMetrics()
    {
        ...
        // context.update("dataverseForm");
        PrimeFaces.current().ajax().update("dataverseForm");
    }

    public String getSubjects()
    {
        if (getDataverseId() != null) {
            logger.log(Level.WARNING, "<------Dataverse:" + getDataverseId());
        } else {
            logger.log(Level.WARNING, "<------Dataverse es null :" + getDataverseId());
        }
        String response = "['Subject','Views', 'Downloads'],";
        List<MetricResult> res = getListSubjects();
        for (MetricResult metric : res) {
    ...
    public List<MetricResult> getListSubjects()
    {
        List<MetricResult> res = new LinkedList();
        try
        {
            String sanitizedToyyyymm = MetricsUtil.sanitizeYearMonthDayUserInput(this.dateEnd);
            String sanitizedFromyyyymm = MetricsUtil.sanitizeYearMonthDayUserInput(this.dateStart);
            List<Object[]> resultado = null;
            if (getDataverseId() == null) {
                resultado = this.metricsSvc.datasetsMetricsSubject(sanitizedFromyyyymm, sanitizedToyyyymm);
            } else {
                resultado = this.metricsSvc.datasetsMetricsSubject(sanitizedFromyyyymm, sanitizedToyyyymm, getDataverseId());
            }
            for (Object[] objectArray : resultado)
            {
                String subject = objectArray[0] + "";

                long views = 0L;long downloads = 0L;
                if ((objectArray[1] instanceof Long)) {
                    views = ((Long)objectArray[1]).longValue();
                } else if ((objectArray[1] instanceof Integer)) {
                    views = new Long(((Integer)objectArray[1]).intValue()).longValue();
                }
                if ((objectArray[2] instanceof Long)) {
                    downloads = ((Long)objectArray[2]).longValue();
                } else if ((objectArray[2] instanceof Integer)) {
                    downloads = new Long(((Integer)objectArray[2]).intValue()).longValue();
                }
                res.add(new MetricResult(subject, views, downloads));
            }
        }
        catch (Exception e)
        {
    ...
    public List<MetricResult> getListNewDatasets()
    {
        List<MetricResult> res = new LinkedList();
        try
        {
            ...
            List<Object[]> resultado = null;
            if (getDataverseId() == null) {
                resultado = this.metricsSvc.datasetsNewsGroupingDate(sanitizedFromyyyymm, sanitizedToyyyymm);
            } else {
                resultado = this.metricsSvc.datasetsNewsGroupingDate(sanitizedFromyyyymm, sanitizedToyyyymm, getDataverseId());
            }
            for (Object[] objectArray : resultado)
            {
                long count = 0L;
                if ((objectArray[0] instanceof Long)) {
                    count = ((Long)objectArray[0]).longValue();
                } else if ((objectArray[0] instanceof Integer)) {
                    count = new Long(((Integer)objectArray[0]).intValue()).longValue();
                }
                Double year = (Double)objectArray[1];
                Double month = (Double)objectArray[2];
                String date = (year + "-" + month).replace(".0", "");
                res.add(new MetricResult(date, count, 0L));
            }
        }
        catch (Exception e)
        {
    ...
    {
        String response = "['Date','Total'],";
        List<MetricResult> res = getListNewDatasets();
        for (MetricResult metric : res) {
            response = response + "['" + metric.getName() + "'," + metric.getViews() + "],";
        }
        if (response.endsWith(",")) {
            response = response.substring(0, response.length() - 1);
        }
    ...
    public List<MetricResult> getListFilesMostDownloaded()
    {
        ...
        try
        {
            ...
            List<Object[]> resultado = null;
            if (getDataverseId() == null) {
                resultado = this.metricsSvc.filesMostDownloaded(sanitizedFromyyyymm, sanitizedToyyyymm, 20);
            } else {
                resultado = this.metricsSvc.filesMostDownloaded(sanitizedFromyyyymm, sanitizedToyyyymm, getDataverseId(), 20);
            }
            for (Object[] objectArray : resultado)
            {
                long count = 0L;
                ...
                String name = objectArray[2] + "";
                String uri = objectArray[3] + "";
                res.add(new MetricResult(name, count, 0L, uri));
            }
        }
        catch (Exception e)
        {
    ...
    public String getFilesMostDownloaded()
    {
        String response = "['File','Total'],";
        List<MetricResult> res = getListNewDatasets();
        for (MetricResult metric : res) {
            response = response + "['" + metric.getName() + "'," + metric.getViews() + "],";
        }
    ...
    {
        if (getDataverseId() == null) {
            return this.dataverse.getName();
        }
        return this.dataverseService.find(getDataverseId()).getName();
    } 
=======
>>>>>>> dataverse-5.3/master
}
