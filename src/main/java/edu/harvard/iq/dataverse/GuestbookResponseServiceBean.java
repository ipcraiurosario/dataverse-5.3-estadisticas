...
<<<<<<< HEAD
    public Long getCountViewGuestbookResponsesByDatasetId(Long datasetId)
    {
        Query query = this.em.createNativeQuery("select count(o.id) from GuestbookResponse  o  where o.datafile_id is null and o.dataset_id  = " + datasetId);
        logger.log(Level.WARNING, "|||||" + datasetId + " tiene views:" + query.getSingleResult());
        return (Long)query.getSingleResult();
    }

    public GuestbookResponse initGuestbookResponseForView(DatasetVersion workingVersion, DataverseSession session, String IP)
    {
        Dataset dataset = workingVersion.getDataset();
