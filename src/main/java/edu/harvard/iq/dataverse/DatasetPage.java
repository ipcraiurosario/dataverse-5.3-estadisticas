...
<<<<<<< HEAD
import edu.harvard.iq.dataverse.location.IPLocationService;
import edu.harvard.iq.dataverse.location.MetricResult;
import edu.harvard.iq.dataverse.metrics.MetricsServiceBean;
import edu.harvard.iq.dataverse.metrics.MetricsUtil;
import edu.harvard.iq.dataverse.orcid.Vivo;
=======
...
<<<<<<< HEAD
    ...
    private Date dateStart = MetricsUtil.getSixMonthsAgo();
    private Date dateEnd = MetricsUtil.getCurrentDate();
    @EJB
    MetricsServiceBean metricsSvc;
    private Vivo vivoData;
    ...
    public void updateMetrics() {
        ...
        // final context = PrimeFaces.current();
        // context.update("datasetForm");
        // context.ajax().update("datasetForm");
        PrimeFaces.current().ajax().update("datasetForm");
    }


    public void saveView() {
        final javax.servlet.http.HttpServletRequest request = (javax.servlet.http.HttpServletRequest)
                FacesContext.getCurrentInstance().getExternalContext().getRequest();
        String ipAddress = request.getHeader("X-FORWARDED-FOR");
        if (ipAddress == null) {
            ipAddress = request.getRemoteAddr();
        }
        final GuestbookResponse rb = this.guestbookResponseService.initGuestbookResponseForView(this.workingVersion, this.session, ipAddress);
        rb.setGeoIP(IPLocationService.getLocation(ipAddress));
        this.guestbookResponseService.save(rb);
    }

    public String getCountry() {
        String response = "['Country','Views', 'Downloads'],";
        final List<MetricResult> res = this.getListCountry();
        if (res.size() == 0) {
            return "['Country','Views']";
        }
        for (final MetricResult metric : res) {
            response = response + "['" + metric.getName() + "'," + metric.getViews() + "," + metric.getDownloads() + "],";
        }
        if (response.endsWith(",")) {
            response = response.substring(0, response.length() - 1);
        }
        DatasetPage.logger.log(Level.WARNING, "*****getCountry:" + response);
        return response;
    }

    public List<MetricResult> getListCountry() {
        final List<MetricResult> res = new LinkedList<MetricResult>();
        try {
            final String sanitizedToyyyymm = MetricsUtil.sanitizeYearMonthDayUserInput(this.dateEnd);
            final String sanitizedFromyyyymm = MetricsUtil.sanitizeYearMonthDayUserInput(this.dateStart);
            final List<Object[]> resultado = (List<Object[]>)this.metricsSvc.datasetsMetricsToMonthGroupingCountry(this.dataset.getId() + "", sanitizedFromyyyymm, sanitizedToyyyymm);
            for (final Object[] objectArray : resultado) {
                final long views = (long)objectArray[0];
                final long downloads = (long)objectArray[1];
                final String country_name = (String)((objectArray[3] != null) ? objectArray[3] : "");
                res.add(new MetricResult(country_name, views, downloads));
            }
        }
        catch (Exception e) {
            e.printStackTrace();
    ...
    public String getMonth() {
        String response = "['Date','Views', 'Downloads'],";
        final List<MetricResult> res = this.getListMonth();
        if (res.size() == 0) {
            return "['Date','Views']";
        }
        for (final MetricResult metric : res) {
    ...
    public List<MetricResult> getListMonth() {
        final List<MetricResult> res = new LinkedList<MetricResult>();
        try {
            final String sanitizedToyyyymm = MetricsUtil.sanitizeYearMonthDayUserInput(this.dateEnd);
            final String sanitizedFromyyyymm = MetricsUtil.sanitizeYearMonthDayUserInput(this.dateStart);
            final List<Object[]> resultado = (List<Object[]>)this.metricsSvc.datasetsMetricsToMonthGroupingDate(this.dataset.getId() + "", sanitizedFromyyyymm, sanitizedToyyyymm);
            for (final Object[] objectArray : resultado) {
                final long views = (long)objectArray[0];
                final long downloads = (long)objectArray[1];
                final Double year = (Double)objectArray[2];
                final Double month = (Double)objectArray[3];
                final String date = (year + "-" + month).replace(".0", "");
                res.add(new MetricResult(date, views, downloads));
            }
        }
        catch (Exception e) {
            e.printStackTrace();
    ...
    public String getCountViews() {
        try {
            final String sanitizedToyyyymm = MetricsUtil.sanitizeYearMonthDayUserInput(this.dateEnd);
            final String sanitizedFromyyyymm = MetricsUtil.sanitizeYearMonthDayUserInput(this.dateStart);
            final Long resultado = this.metricsSvc.datasetsMetricsToMonth(this.dataset.getId() + "", sanitizedFromyyyymm, sanitizedToyyyymm, "view");
    ...
    public String getCountDownloads() {
        try {
            final String sanitizedToyyyymm = MetricsUtil.sanitizeYearMonthDayUserInput(this.dateEnd);
            final String sanitizedFromyyyymm = MetricsUtil.sanitizeYearMonthDayUserInput(this.dateStart);
            final Long resultado = this.metricsSvc.datasetsMetricsToMonth(this.dataset.getId() + "", sanitizedFromyyyymm, sanitizedToyyyymm, "download");
            return resultado.toString();
        }
        catch (Exception e) {
            e.printStackTrace();
    ...
    public List<MetricResult> getListFilesMostDownloaded() {
        final List<MetricResult> res = new LinkedList<>();
        try {
            final String sanitizedToyyyymm = MetricsUtil.sanitizeYearMonthDayUserInput(this.dateEnd);
            final String sanitizedFromyyyymm = MetricsUtil.sanitizeYearMonthDayUserInput(this.dateStart);
            final List<Object[]> resultado = (List<Object[]>)this.metricsSvc.filesMostDownloaded(this.dataset.getId() + "", sanitizedFromyyyymm, sanitizedToyyyymm, 20);
            for (final Object[] objectArray : resultado) {
                final long count = (long)objectArray[0];
                final String name = objectArray[2] + "";
                res.add(new MetricResult(name, count, 0L));
            }
        }
        catch (Exception e) {
            e.printStackTrace();
